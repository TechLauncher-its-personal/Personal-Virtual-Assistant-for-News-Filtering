...
router.get('/login', (req, res, next) => {
    ...
    res.render('login', {
        ...
        page_title: req._("Login | ISS Personal Assistant")
    });
});
...
router.get('/2fa/setup', userUtils.requireLogIn, iv.validateGET({ force: 'boolean' }), (req, res, next) => {
    if (req.user!.totp_key !== null && req.query.force !== '1') {
        res.status(400).render('error', {
            page_title: req._("Error | ISS Personal Assistant"),
    ...
    res.render('2fa_setup', {
        page_title: req._("Two-Factor Authentication | ISS Personal Assistant"),
        encryptedKey,
        qrUrl
    });
});
...
router.get('/register', (req, res, next) => {
    res.render('register', {
        csrfToken: req.csrfToken(),
        page_title: req._("Register | ISS Personal Assistant"),
    });
});
...
router.get('/verify-email/:token', userUtils.requireLogIn, (req, res, next) => {
    db.withTransaction(async (dbClient) => {
        ...
        res.render('email_verified', {
            page_title: req._("Verification Successful | ISS Personal Assistant")
        });
    }).catch(next);
});
...
router.post('/resend-verification', userUtils.requireLogIn, (req, res, next) => {
    ...
    sendValidationEmail(req.user!.cloud_id, req.user!.username, req.user!.email).then(() => {
        res.render('message', {
            page_title: req._("Verification Sent | ISS Personal Assistant"),
...
router.get('/recovery/start', (req, res, next) => {
    res.render('password_recovery_start', {
        page_title: req._("Password Reset | ISS Personal Assistant")
    });
});
...
router.post('/recovery/start', iv.validatePOST({ username: 'string' }), (req, res, next) => {
    db.withClient(async (dbClient) => {
        ...
        if (users.length === 0) {
            ...
            res.render('message', {
                page_title: req._("Password Reset Sent | ISS Personal Assistant"),
